    ...
    def __init__(self, opt):
        ...
        if opt.label_rate is not None:
            ...
            for name in self.img_names:
                if name not in self.label_img_names:
                    self.weak_names.append(name)
            self.weak_t1_paths = [file_root + '/' + opt.phase + '/A/' + x for x in self.weak_names]
            self.weak_t2_paths = [file_root + '/' + opt.phase + '/B/' + x for x in self.weak_names]
            self.weak_label_paths = [file_root + '/' + opt.phase + '/label/' + x for x in self.weak_names]
